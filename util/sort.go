...
func StrToSort(str string) (sort string) {
	// 获取中文字符串第一个字符
	for _, v := range []rune(str) {
		if unicode.Is(unicode.Han, v) {
			//汉字
			sort += HanToPinYin(string(v))
		} else if unicode.IsLetter(v) {
			//字母
			sort += strings.ToUpper(string(v))
		} else if unicode.IsDigit(v) {
			//数字
			sort += DigitToLetter(string(v))
...
func HanToPinYin(han string) string {
	// Utf8 转 GBK2312
	firstCharGbk, err := Utf8ToGbk([]byte(han))
	if err != nil {
		return ""
	}
	// 获取第一个字符的16进制
...
func DigitToLetter(digit string) string {
	if digit == "0" {
		return "a"
	}
	if digit == "1" {
